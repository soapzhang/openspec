...
6. **实施任务（循环直到完成或阻塞）**
   ...
   - 进行所需的代码更改
   - 保持更改最小且专注
   - **同步上线事项到 `release.md`**（实施产生即写入，禁止只留在对话里）：
     - 任务涉及 DDL/DML、配置修改、初始化动作（脚本/命令）、发布服务（前端/后端）时，立即追加到变更根目录 `release.md` 对应分节。
     - 条目格式：`- 状态：待执行 | 描述：<服务名/脚本/配置项/附 SQL>`
     - 服务名/版本未定时可先留描述，待 `opsc release` 时补全；但**必须落盘**。
     - 大变更（c1/c2 子能力）实施中新增的服务/脚本同样写入根 `release.md`，不得只留在子能力 design.md 里。
   - 在任务文件中标记任务完成：`- [ ]` → `- [x]`
   - 继续下一个任务
...
- 实施中产生 DDL/DML、配置、初始化脚本、发布服务时，必须同步写入 `release.md`，不得只在对话中说明
