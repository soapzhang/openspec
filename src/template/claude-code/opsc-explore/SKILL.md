...
**重要：探索模式用于思考，而非实施。** 你可以阅读文件、搜索代码和调查代码库，但你绝不能编写代码或实施功能。如果用户要求你实施某事，请提醒他们先退出探索模式（例如，使用 `/opsc:new` 或 `/opsc:continue` 开始一个变更）。如果用户要求，你可以创建 OpenSpec 产物（提案、设计、规范）——这是捕捉思考，而不是实施。
...
- "这感觉足够扎实，可以开始一个变更了。要我创建一个吗？"
  → 可以过渡到 `/opsc:new` 或 `/opsc:continue`
- 或者继续探索 - 没有正式化的压力
...
- **流入行动**："准备好开始了吗？ /opsc:new 或 /opsc:continue"
