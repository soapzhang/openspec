...
export function getToolsWithSkillsDir(): string[] {
  return AI_TOOLS.filter((t) => t.skillsDir && t.available !== false).map((t) => t.value);
}
...
export function getToolStates(projectRoot: string): Map<string, ToolSkillStatus> {
  ...
  const toolIds = AI_TOOLS.filter((t) => t.skillsDir && t.available !== false).map((t) => t.value);
...
export function getConfiguredTools(projectRoot: string): string[] {
  return AI_TOOLS
    .filter((t) => t.skillsDir && t.available !== false && getToolSkillStatus(projectRoot, t.value).configured)
    .map((t) => t.value);
}
